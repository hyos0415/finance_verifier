...
| 4 | (없음) | **Macro F1은 진단 지표로는 유용하지만 대표 선택 지표로는 부적절하다.** 클래스별 실패는 정확히 짚어내지만, 6%짜리 클래스가 macro 평균의 33%를 차지해 모델 순위를 뒤집는다 |
...
**먼저 분명히 해둘 것: Macro F1이 이번에 쓸모없는 신호를 준 게 아니다.** 오히려 Qwen이 세 클래스 중
하나를 통째로 버리고 있다는 사실을 강하게 벌점으로 줬다. Pilot에서 이렇다.
...
Macro F1은 여기서 **"세 클래스 중 하나를 완전히 버린 분류기를 정말 더 좋다고 할 수 있나"**라고 묻고
있다. 그 문제 제기 자체는 타당하고, 실제로 이번 감사에서 §① 발견의 실마리가 됐다.

**문제는 가중치다.** INSUFFICIENT는 Pilot 64건 중 4건(6%)인데 3클래스 macro 평균에서는 33.3%를
차지한다. 비용 구조도 대칭이 아니다 — 금융 Verifier에서 "정보 부재를 충돌로 오판"하는 비용과
"틀린 답을 승인"하는 비용은 같지 않은데, macro 평균은 둘을 같은 무게로 센다.

Test에서는 표본이 더 작아 왜곡이 커진다.
...
verdict 1건이 macro를 0.22 움직이는데 정확도로는 1.9%p 차이다. **최종 수치의 기준은 실제 채택한
서빙 경로로 정정한다.**

**정확한 결론**: Macro F1은 **클래스별 실패를 드러내는 진단 지표로는 유용하다.** 다만 클래스 불균형이
크고 비용 구조가 비대칭인 이 태스크에서 **모델 선정의 대표 지표로 쓰기에는 부적절하다.** 클래스별
recall과 support를 함께 제시하고, 대표 지표는 비용 구조를 반영한 제약식(③)으로 두는 편이 맞다.
