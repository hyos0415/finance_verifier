...
2. **Macro F1은 진단 지표이지 대표 선택 지표가 아니다.** 잘못된 신호를 준 게 아니라 오히려
   "Qwen이 세 클래스 중 하나를 버렸다"를 정확히 벌점으로 줬다. 문제는 가중치다 — 전체의 6%인
   INSUFFICIENT가 macro 평균에서 33%를 차지해 FAR·정확도가 모두 나은 모델을 더 낮게 매긴다.
   클래스별 실패 진단에는 계속 쓰되, 모델 선정의 대표 지표로는 부적절하다.

### Qwen 선택을 유지할 수 있는가 — 현재 위험 우선순위에서는 유지한다

먼저 엄격하게 짚으면, 채택 설정의 Qwen은 **완전한 3분류 Verifier라고 부르기 어렵다** — SUPPORTED
Recall 1.000 / UNSUPPORTED 0.8846 / **INSUFFICIENT 0.000**이다. 요구사항이 "세 판정을 정확히
구별한다"였다면 탈락시키는 게 맞다.

이 프로젝트가 명시적으로 우선한 위험은 다르다 — **잘못된 금융 정보를 승인하는 False Accept의
최소화**다. 그 기준 아래에서 같은 프롬프트·같은 Pilot 64건을 정면 비교하면 이렇다.
...
**정리하면**: "Qwen이 객관적으로 더 나은 Verifier다"가 아니라, **"이 프로젝트가 정한 위험
우선순위에서는 Qwen을 유지할 근거가 있다"**가 정확한 서술이다. 운영 요구가 "세 판정을 정확히
구별한다"로 바뀌면 결론도 바뀐다.

**당시 근거는 불완전했다.** Macro F1을 "보조 지표"라며 넘긴 게 결과적으로 나쁘지 않았던 건 그 지표를
*대표 지표*로 쓰기 어려워서였지, 거기 담긴 신호가 무의미해서가 아니었다 — 그 신호는 "Qwen이 클래스
하나를 버렸다"를 정확히 가리키고 있었다.
