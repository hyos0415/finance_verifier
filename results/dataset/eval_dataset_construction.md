...
3. **확정된 약점 2종** — Test에서 ① INSUFFICIENT↔UNSUPPORTED 경계 혼동, ② `condition_omission`
   (AND 복합조건 일부만 인용하면 누락을 못 잡음)이 재현됐다. 특히 ②는 Pilot에서 "어려운 문항"으로
   관찰된 게 **새로운 상품·새로운 문항에서 2/2 모두 그대로 재현**되며 우연이 아님이 확인됐다.
4. **모델 종속성 검증 (cross-model)** — 같은 Test 53건을 훨씬 큰 모델에 돌려 실패가 Qwen 고유인지
   확인했다. Nemotron Ultra 550B의 지표는 Qwen3.5-4B와 **완전히 동일**했다(파라미터 130배 차이).
   즉 위 약점은 모델 체급 문제가 아니라 **태스크와 판정 경계 정의 자체의 구조적 한계**라는 결론의 근거가 됐다.

| 모델 | Test FAR | UNSUPPORTED Recall | Macro F1 |
|---|---|---|---|
