...
> **⚠️ 이후 정정(#28).** 위 "구조적으로 어렵다"는 해석은 대조군을 4개 모델로 넓히면서 뒤집혔다 —
> 절차형 프롬프트를 주면 Qwen도 2/2를 잡아내고, Sonnet·Nemotron에서는 전체 지표가 오히려 개선된다.
> 병목은 "누락 대조 자체가 어렵다"가 아니라 **조건들이 ALL_OF인지 ANY_OF인지 범위를 잡는 능력**이며,
> 이 능력이 없는 모델에서는 같은 프롬프트가 정상 claim까지 거부한다.
> 상세: [`precondition_audit.md`](./precondition_audit.md).
