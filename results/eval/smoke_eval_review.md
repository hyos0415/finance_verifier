...
### 사람이 만든 버그로 밝혀진 "공유 오답" 하나

두 모델이 똑같이 틀린 `p013_c03_2`(제주은행 J정기예금 term_error 시나리오)를 분석하다가, evidence는
"20백만원"인데 내가 시나리오 instruction을 쓸 때 "2천만원"으로 바꿔 쓴 걸 발견했다 — 수학적으론 같은
금액(20,000,000원)인데 표기 단위가 달라 두 모델 다 "다른 숫자"로 인식하고 UNSUPPORTED로 답했다.
**모델의 추론 실패가 아니라 시나리오 작성 실수**다. `p018` 상품 전체에서 Kanana만 유독 많이 틀린
것(mutex 관련 7건 중 7건)을 보고 "①②③④⑤ 원문자 포맷이 문제 아닐까" 의심했었는데, Qwen은 같은 포맷의
claim 대부분(7건 중 5건)을 맞혀서 포맷 자체가 원인이라는 가설은 이 데이터로는 확인되지 않았다 — 이
공유 오답의 진짜 원인은 포맷이 아니라 단위 불일치였다.
...
3. **`p013_c03_2`는 시나리오 재작성 대상** — "20백만원"을 evidence 그대로 써서 다시 생성할지, 아니면
   그냥 제외할지 결정.
4. 숫자 표기 정규화 전처리는 보류 — 재현 빈도가 늘어나면 그때 규칙 기반으로 추가.
5. v3 프롬프트는 계속 보류 — 필요해지면 그때 구체적 타겟을 다시 잡는다.
6. Dev/Test 단계로 갈 때 Qwen의 INSUFFICIENT↔UNSUPPORTED 혼동을 별도 실패 유형으로 추적.
