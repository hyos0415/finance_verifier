...
- `data/smoke/claim_dataset.json` — atomic claim (decomposition 후, self-containment 수정 후 27개 — 아래 "재오픈" 절 참고. 최초 실행 시엔 29개였음)
...
## 재오픈 — #14 Eval Harness에서 발견한 self-containment 결함 수정

#14에서 Qwen/Kanana를 이 29개 claim에 돌려봤더니, 두 모델이 똑같이 틀리는 케이스 중 일부가 **Verifier
잘못이 아니라 이 decomposer의 결함**으로 드러났다 — decompose된 claim이 원본 답변의 앞선 문장을
가리키는 대명사/지시어("두 조건", "해당")를 그대로 남기거나, 숫자에 붙어있던 시점 조건("1개월 이내")을
빠뜨린 채로 나온 것. Verifier는 claim을 하나씩 독립적으로 보기 때문에, claim 안에 없는 정보를
참조하는 문장은 원리적으로 판단이 불가능하다 — 아무리 Verifier 프롬프트를 고쳐도 못 푸는 문제라
decomposer 쪽 원인을 고쳤다.

### 무엇을 바꿨나

`claim_decomposer.py`의 시스템 프롬프트에 "self-contained해야 한다"는 원칙을 구체적인 금지 규칙으로
명시했다:
- 다른 claim을 가리키는 대명사/지시어("두 조건", "해당", "이 금리", "그 조건") 금지 — 지칭 대상을
  풀어써야 함
- 숫자·사실에 붙은 조건(시점, 자격, 상품 종류)을 빼먹지 말 것 — 원문이 "1개월 이내에는 50%"라고 하면
  그 50%를 언급하는 모든 claim에 "1개월 이내"가 같이 있어야 함

동일한 `synthetic_answers.json`(답변 텍스트는 재생성 안 함)에 대해 decompose만 다시 돌렸다.

### Before → After

| 케이스 | 이전 | 이후 |
|---|---|---|
| p003_c01 (OR 조건) | "두 조건 중 하나만 충족해도 된다" — 지시어만 있고 대상이 안 나옴 | "전월 총수신 평잔이 30만원 이상인 조건과 상품 가입 전에 첫만남플러스통장을 보유한 조건은 둘 중 하나만 충족해도 우대금리 적용 대상이 된다" — 완전히 풀어씀 |
| p003_c02 (AND 반전) | "두 조건을 동시에 충족하면 우대금리가 적용된다" — 별도 claim으로 분리돼 있었음 | "전월 총수신 평잔이 30만원 이상이면서 동시에 상품 가입 전 첫만남플러스통장을 보유한 경우에는 연 0.10%p의 우대금리가 적용된다" — 조건과 결과를 하나의 완결된 문장으로 병합 |
| p001_c01 (mtrt_int 시점조건) | "만기후이자는 만기시점 약정이율의 50%이다" — "1개월 이내" 시점 조건이 빠짐 | "만기 후 1개월 이내에 해지할 경우 만기후이자로 만기시점 약정이율의 50%가 적용된다" — 시점 조건이 숫자와 한 문장에 유지됨 |

부수 효과로, p003_c02의 병합된 문장이 조건("동시에 충족")과 결과("우대금리 적용")를 하나로 합치면서
**이전에 남아 있던 필요조건/충분조건 애매함(`p003_c02_5`)도 같이 해소됐다** — 더 이상 조건과 결과를
분리해서 서술하지 않으니 애매하게 잘릴 여지가 없다.

### 결과

29개 → **27개** claim (더 완결된 문장으로 병합되며 개수는 줄었다). `needs_manual_review` 여전히
0/27 — `error_match` 규칙이 새 문구에서도 그대로 작동함. 전수 확인 결과 지시어/대명사가 남은 claim
없음.

## 다음 단계로 넘길 것 (#13/#14에서 다룰 화두)

1. Pilot 규모로 확장할 때 `error_match`를 시나리오별로 더 폭넓게(대체 표현 여러 개) 정의하는 관행을 유지.
2. ~~`p003_c02_5` 같은 필요/충분조건 뉘앙스 케이스~~ — self-containment 수정으로 해소됨(위 참고).
3. Atomicity 체크가 이번 표본에서 한 번도 안 걸렸으므로, 실제로 비atomic한 사례를 Pilot 단계에서 최소
   1개는 포함해 규칙이 실제로 작동하는지 확인 필요.
4. #14의 claim_dataset.json 기준 eval 결과(prompt-v1/v2)는 이 재분해로 전부 무효 — #14 브랜치에서
   새 baseline으로 다시 잡아야 함.
