...
1단계(환경 구성 + raw snapshot)부터 #14(Eval Harness + Pilot 모델 선정)·#23(Test 데이터셋 구축 + 최종 eval)·#25(Qwen Latency 개선 실험)·#15(최종 결과 정리)까지 전부 완료 — **Qwen3.5-4B-int4-AutoRound를 유일한 Verifier 후보로 확정**했고(Kanana는 비교 대상에서 제외), 프롬프트는 v2로 Test 단계까지 고정했다(`results/eval/smoke_eval_review.md`의 "모델 선정 확정" 절, `results/eval/test_eval_review.md` 참고). Test(53건, held-out)에서 약점 두 가지가 정리됨: ①INSUFFICIENT↔UNSUPPORTED 경계 혼동, ②`condition_omission`(AND 복합조건 일부 인용 시 누락 못 잡음). **단, 이 두 약점의 해석은 이후 #28에서 정정됐다 — 아래 참고.** #25에서 CUDA Graph 활성화로 batch=1 decode ~3.6배 개선을 확인하고 서빙 설정을 `--max-num-seqs=4`로 채택했다(`results/model_selection/qwen_latency_diagnosis.md`의 "이슈 #25 실측 결과" 절 참고). #15에서 위 내용을 `results/final/report.md`(기술 리포트)로 정리하고, 비전문가용 발표 자료(`results/final/report_dashboard.html`)와 `README.md` 개편까지 마쳤다. **1주 과제 스코프의 전체 작업이 완료됐다.**

그 뒤 **#28에서 `eval-precondition` 스킬로 eval 설계를 자체 감사하고 4개 모델(Qwen·Haiku·Sonnet·Nemotron 550B)로 재검증해, 리포트 결론 두 개를 정정했다** (`results/eval/precondition_audit.md`가 source of truth):

1. **①은 태스크 한계가 아니라 모델별 특성이다.** 같은 프롬프트로 Kanana(로컬 3B)·Haiku·Sonnet은 INSUFFICIENT를 4/4 맞힌다. Qwen·Nemotron·Gemma만 실패한다. 대조군이 2개뿐이어서 "체급 무관한 구조적 한계"로 잘못 일반화했던 것.
2. **②는 프롬프트로 통제는 되지만 안정적이지 않다.** 절차형 완전성 규칙을 넣으면 Test 2/2를 잡지만, 채택 모델 Qwen에서 정상 claim 인식률이 0.913→0.674로 붕괴한다(Sonnet·Nemotron에서는 개선). 병목은 "누락 미탐지"가 아니라 **조건이 ALL_OF인지 ANY_OF인지, 어느 혜택에 속하는지 범위를 잡는 능력**이다.

부수적으로 **평가 지표 설계 결함 두 가지**도 드러났다 — (a) 지표 우선순위(FAR→Recall→Schema)에 utility 제약이 없어 `"UNSUPPORTED"` 상수 스텁이 1·2·3순위를 전부 이긴다 → 순위제 대신 제약식(FAR을 낮추되 FRR을 악화시키지 않는다)이 맞다. (b) Macro F1은 클래스별 진단에는 유용하지만 6%짜리 클래스가 macro 평균의 33%를 차지해 모델 선정 대표 지표로는 부적절하다. **production 프롬프트(v6=v2 내용)와 Qwen 선정은 유지했다** — Qwen의 INSUFFICIENT 실패는 100% UNSUPPORTED로 가는 보수적 방향이고, 이 프로젝트가 우선한 위험(False Accept 최소화)에서는 근거가 성립한다.

**eval 실행 로그(`results/eval/raw`, `results/eval/audit`)는 evidence 원문 인용이라 `.gitignore`이고, 텍스트를 제거한 `results/eval/runs_summary.json`(24개 실행)만 추적한다** — repo의 모든 인용 수치는 이 파일로 검증 가능하다. 프롬프트 실험은 Langfuse 별도 라벨(`experiment`=v7, `experiment2`=v8)로만 올렸고 production은 v6 그대로다. 세 하니스 전부 `--prompt-label`로 production을 건드리지 않고 변형을 A/B할 수 있다.

이후 착수할 항목은 CLAUDE.md "지금 하지 않는 것" 절과 `results/final/report.md` §11(다음 실험 후보)에 후속 과제로 남겨둔다.

각 작업 블록은 GitHub 이슈로 트래킹한다: [#1](../../issues/1)(완료, repo/secret) · [#2](../../issues/2)(완료, 모델 로드 체크) · [#3](../../issues/3)(완료, WSL2/vLLM) · [#4](../../issues/4)(완료, API 첫 호출) · [#5](../../issues/5)(완료, 데이터 프로파일링을 Eval Design 관점으로 재검토) · [#6](../../issues/6)(완료, canonical schema / Claim metadata) · [#7](../../issues/7)(완료, 모델 serving smoke — 심화) · [#12](../../issues/12)(완료, Claim Decomposer 구현) · [#13](../../issues/13)(완료, Verifier Client 구현) · [#14](../../issues/14)(완료, Eval Harness 구현 + Pilot 모델 선정 — Qwen 확정) · [#23](../../issues/23)(완료, Test 데이터셋 구축 — unseen, Claude+Codex 공동 생성, 확정 약점 2종 발견) · [#25](../../issues/25)(완료, Qwen Latency 개선 실험 — CUDA graph 활성화로 3.6배 개선, max-num-seqs=4 채택) · [#15](../../issues/15)(완료, 최종 결과 정리 — 기술 리포트 + 발표용 대시보드 + README 개편) · [#28](../../issues/28)(완료, eval 전제 자체 감사 + 크로스모델 검증 — 리포트 결론 2건 정정, PR #29).
